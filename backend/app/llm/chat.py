...
# คำที่คนพูดกับคำที่เว็บใช้จริงไม่ตรงกัน — นับจาก DB แล้วพบว่าคำฝั่งซ้ายไม่โผล่เลยสักครั้งใน 373 chunk
# การค้นแบบตรงคำจึงพลาดทั้งหมด เคสจริงที่เจอ: ถาม "ค่าเทอม SE เท่าไหร่" ระบบหยิบเอกสาร SE มาถูกแล้ว
# แต่ได้ chunk ที่ไม่มีตัวเลข บอทเลยตอบว่าไม่มีข้อมูล ทั้งที่อยู่ใน DB จริง (40,000 บาท/ภาคการศึกษา)
_SITE_VOCABULARY = (
    "ค่าเทอม / ค่าเรียน → เว็บใช้ \"ค่าธรรมเนียมการศึกษา\" (คำว่า \"ค่าเทอม\" ไม่มีในเว็บเลย)\n"
    "สมัครยังไง / คุณสมบัติผู้สมัคร → \"การรับสมัคร\" \"สมัครเรียน\" \"Portfolio\" \"TCAS\"\n"
    "เรียนกี่ปี / เรียนนานไหม → \"ระยะเวลาการศึกษา\" \"หน่วยกิต\"\n"
    "จบแล้วทำงานอะไร → \"อาชีพ\"\n"
    "เปิดกี่โมง / ปิดกี่โมง → \"เวลาทำการ\"\n"
    "เบอร์โทร / ติดต่อยังไง → \"โทร\" \"ติดต่อ\" \"ที่อยู่\""
)
...
_EXPAND_SYSTEM_PROMPT = (
    ...
    "คำที่คนถามมักไม่ตรงกับคำที่เว็บใช้ ถ้าคำถามเข้าข่ายด้านล่างนี้ ให้ใส่คำฝั่งขวาเป็นคำค้นด้วยเสมอ "
    "(ใส่เพิ่ม ไม่ใช่ใส่แทน — คำเดิมของผู้ถามก็ยังใส่ได้):\n" + _SITE_VOCABULARY + "\n"
...
def expand_search_terms(
    ...
    history: บทสนทนาก่อนหน้าเป็นคู่ (คำถาม, คำตอบ) เรียงเก่าไปใหม่
    ใส่เข้ามาเพื่อให้คำถามต่อเนื่องที่ไม่ครบใจความอย่าง "แล้วค่าเทอมล่ะ" แตกคำค้นได้ถูก
    (ต้องรู้จากบทสนทนาก่อนหน้าว่ากำลังพูดถึงหลักสูตรไหน) — ไม่ต้องเรียก LLM เพิ่มเลย
    เพราะเป็นการเติมข้อมูลเข้าไปในรอบที่เรียกอยู่แล้ว
    """
    if history:
        previous = "\n".join(f"ถาม: {q}\nตอบ: {a}" for q, a in history)
        user_message = (
            f"บทสนทนาก่อนหน้า:\n{previous}\n\n"
            f"คำถามล่าสุด: {question}\n"
            "ถ้าคำถามล่าสุดไม่ครบใจความและอ้างถึงเรื่องที่คุยกันไปแล้ว "
            "(เช่น ถามว่า \"แล้วค่าเทอมล่ะ\" หลังจากเพิ่งคุยเรื่องหลักสูตรหนึ่ง) "
            "ให้เติมชื่อหลักสูตร/หัวข้อจากบทสนทนาก่อนหน้าเข้าไปในคำค้นด้วย"
        )
    else:
        user_message = question

    try:
        raw = llm.complete(_EXPAND_SYSTEM_PROMPT, user_message, max_tokens=150)
    except Exception:
